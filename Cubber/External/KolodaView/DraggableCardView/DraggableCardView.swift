...
private var scaleCurrent :CGFloat = 0.8
...
public class DraggableCardView: UIView, UIGestureRecognizerDelegate {
    ...
    private var swipGestureLeft: UISwipeGestureRecognizer!
    private var swipGestureRight: UISwipeGestureRecognizer!
    private var tapGestureRecognizer: UITapGestureRecognizer!
    private var animationDirectionY: CGFloat = 1.0
    private var dragBegin = false
    private var dragDistance = CGPoint.zero
    private var dragDistanceMin = CGPoint.zero
    private var dragDistanceMax = CGPoint.zero
    ...
    private func setup() {
        ...
        swipGestureLeft = UISwipeGestureRecognizer.init(target: self, action: #selector(DraggableCardView.swipeGestureRecognized(_:)))
        swipGestureRight = UISwipeGestureRecognizer.init(target: self, action: #selector(DraggableCardView.swipeGestureRecognized(_:)))
        swipGestureLeft.direction = .left
        swipGestureRight.direction = .right
//        addGestureRecognizer(swipGestureLeft)
//        addGestureRecognizer(swipGestureRight)
        
        
        addGestureRecognizer(panGestureRecognizer)
        swipGestureRight.delegate = self
        swipGestureLeft.delegate = self
    ...
    func swipeGestureRecognized(_ gestureRecognizer: UISwipeGestureRecognizer ) {
        
        let direction :SwipeResultDirection = gestureRecognizer.direction == .left ? .left : .right
        
            let toPoint:CGPoint = animationPointForDirection(direction)
            var transform = CATransform3DIdentity
            transform = CATransform3DScale(transform, 0.62, 0.62, 1)
            transform = CATransform3DRotate(transform, 0, 0, 0, 1)
            transform = CATransform3DTranslate(transform, toPoint.x, 0, 0)
            self.overlayView?.overlayState = direction
            self.overlayView?.alpha = 0.0
            self.delegate?.card(self, wasSwipedIn: direction)
           ...
           /* layer.transform = transform
            ...
            overlayView?.overlayState = SwipeResultDirection.left
            overlayView?.alpha = 0.0
            delegate?.card(self, wasSwipedIn: SwipeResultDirection.left)
            scaleCurrent = 0.85
            //self.superview?.sendSubview(toBack: self)
            ...
            translationAnimation?.toValue = NSValue(cgPoint: animationPointForDirection(SwipeResultDirection.left))
            
            translationAnimation?.animationDidApplyBlock = {(void) in
                self.layer.transform = CATransform3DScale(CATransform3DIdentity, self.layer.transform.m11 - 0.01, self.layer.transform.m11 - 0.01, 1.0)
            }
    ...
    func panGestureRecognized(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            var transform = CATransform3DIdentity
            transform = CATransform3DScale(transform, scaleCurrent, scaleCurrent, 1)
            transform = CATransform3DRotate(transform, 0, 0, 0, 1)
        ...
        case .changed:
            ...
            var transform = CATransform3DIdentity
            let scale = layer.transform.m11 - 0.001 > 0.65 ? layer.transform.m11 - 0.001 : 0.65
            transform = CATransform3DScale(transform,  scale , scale, 1)
            transform = CATransform3DRotate(transform, 0, 0, 0, 1)
            transform = CATransform3DTranslate(transform, dragDistance.x, 0, 0)
            if self.frame.midX > 345 || self.frame.midX < -10 {
    ...
    func swipeAction(_ direction: SwipeResultDirection) {
        
        let toPoint:CGPoint = animationPointForDirection(direction)
        var transform = CATransform3DIdentity
        transform = CATransform3DScale(transform, 0.65, 0.65, 1)
        transform = CATransform3DRotate(transform, 0, 0, 0, 1)
        transform = CATransform3DTranslate(transform, toPoint.x, 0, 0)
        self.overlayView?.overlayState = direction
        self.overlayView?.alpha = 0.0
        self.delegate?.card(self, wasSwipedIn: direction)
        UIView.animate(withDuration: direction == .left ? 0.2 : 0.4, animations: {
            self.alpha = 1.0
            self.layer.transform = transform
        }, completion: { (completed) in
