...
public final class MD5: DigestType {
    ...
    fileprivate func process(block chunk: ArraySlice<UInt8>, currentHash: inout Array<UInt32>) {
        ...
        for j in 0 ..< k.count {
            ...
            let gAdvanced = g << 2
            //Kishan Vyas
            let c1: Int = chunk.startIndex &+ gAdvanced
            let c2: Int = chunk.startIndex &+ gAdvanced &+ 1
            let c3: Int = chunk.startIndex &+ gAdvanced &+ 2
            let c4: Int = chunk.startIndex &+ gAdvanced &+ 3
            var Mg = UInt32(chunk[c1]) | UInt32(chunk[c2]) << 8 | UInt32(chunk[c3]) << 16
                Mg = Mg | UInt32(chunk[c4]) << 24
