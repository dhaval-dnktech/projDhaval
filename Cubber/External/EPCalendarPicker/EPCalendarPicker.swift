...
open class EPCalendarPicker: UICollectionViewController {
    ...
    override open func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        let startDate = Date(year: startYear, month: self.startDate.month(), day: 1)
    ...
    override open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row >= prefixDays {
           ...
           //  cell.lblText.text = "20845"
            if arrSelectedDates.filter({ $0.isDateSameDay(currentDate)
                ...
                if currentDate.isToday() && hightlightsToday {
                    cell.isCellSelectable = true
                    cell.lblDay.textColor = weekdayTintColor
                    //cell.setTodayCellColor(todayTintColor)
                }
                
                if self.minimumDate != nil && Calendar.current.compare(self.minimumDate!, to: currentDate, toGranularity: .day) == .orderedDescending {
                    cell.isCellSelectable = false
                    cell.lblDay.textColor = UIColor.lightGray
                }
           ...
           // cell.lblText.text = "20845"
            ...
            if hideDaysFromOtherMonth {
                cell.lblDay.textColor = UIColor.clear
              //  cell.lblText.textColor = UIColor.clear
            } else {
                cell.lblDay.textColor = self.dayDisabledTintColor
               // cell.lblText.textColor = .green
