...
class InsuranceViewController: UIViewController ,UITextFieldDelegate , UIScrollViewDelegate , UITableViewDelegate,UITableViewDataSource , OperatorViewDelegate , CubberTextFieldDelegate , RegionViewDelegate , AppNavigationControllerDelegate ,VKDatePopoverDelegate  {

    //MARK: CONSTANT
    internal let TAG_PLUS: Int = 100
    internal let TAG_INSURENCE_REGION: Int = 3
    ...
    @IBOutlet var txtInsurance_DOB: UITextField!
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.SetScreenName(name: F_MODULE_INSURANCE, stclass: F_MODULE_INSURANCE)
        self.sendScreenView(name: F_MODULE_INSURANCE)
    ...
    func setValueInOperatorField(dictOperator:typeAliasDictionary , dictRegion:typeAliasDictionary , isOrder:Bool) {
        ...
        if !dictRegion.isEmpty {
            for viewBG in self.viewServiceTextFiled.subviews {
                if viewBG.isKind(of: CubberTextField.self) {
                    let _CubberTextField = viewBG as! CubberTextField
                    if _CubberTextField.txtContent.tag - TAG_PLUS == TAG_INSURENCE_REGION {
    ...
    fileprivate func callGetLatestOrdersService() {
        ...
        let params = [REQ_HEADER:DataModel.getHeaderToken(),
                      ...
                      REQ_ORDER_TYPE_ID: VAL_ORDERTYPE_INSURANCE] as [String : Any]
    ...
    func cubberTextFieldShouldBeginEditing(_ textField: UITextField) {
          ...
          self.resetBillInfo()
        if textField.tag - TAG_PLUS == TAG_INSURENCE_REGION && textField.accessibilityLabel != nil {
        ...
        if  textField.accessibilityLabel == "0" {
            self.txtInsurance_DOB = textField
            self.hideKeyboard()
            self.showDateSelectionPopover(textField)
        }
    }
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        if textField == txtOperator {
            self.hideKeyboard()
            let operatorView:OperatorView = OperatorView.init(frame: UIScreen.main.bounds, categoryID: HOME_CATEGORY_TYPE.INSURANCE.rawValue)
    ...
    func vkDatePopoverSelectedDate(_ vkDate: Date, strDate: String, dateType: DATE_TYPE, dateFormat: DateFormatter) {
        txtInsurance_DOB.text = strDate
    }
    
    func vkDatePopoverClearDate(_ dateType: DATE_TYPE) {
        txtInsurance_DOB.text = ""
