//
//  LandlineAndBroadBandViewController.swift
//  Cubber
//
...
class LandlineAndBroadBandViewController: UIViewController ,UITextFieldDelegate , UIScrollViewDelegate , UITableViewDelegate,UITableViewDataSource , OperatorViewDelegate , CubberTextFieldDelegate , RegionViewDelegate , AppNavigationControllerDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.SetScreenName(name: F_MODULE_LANDLINE, stclass: F_MODULE_LANDLINE)
        self.sendScreenView(name: F_MODULE_LANDLINE)
    ...
    fileprivate func setBillInfo(dict:typeAliasDictionary) {
        ...
        else{ constraintViewBiillInfo_ViewConsumerNameHeight.constant = 0 ; count += 1}
        ...
        else{ constraintViewBiillInfo_ViewBillDueDateHeight.constant = 0 ; count += 1}
        
        if !billDate.isEmpty && billDate != "0" {
            lblBillDate.text = billDate
            constraintViewBiillInfo_ViewBillDateHeight.constant = 30
        }
        else{ constraintViewBiillInfo_ViewBillDateHeight.constant = 0 ; count += 1 }
    ...
    fileprivate func callGetBillAmountSerivice() {
        ...
        let params = [REQ_HEADER:DataModel.getHeaderToken(),
                      REQ_CATEGORY_ID: HOME_CATEGORY_TYPE.LANDLINE.rawValue,
    ...
    fileprivate func callGetLatestOrdersService() {
        ...
        let params = [REQ_HEADER:DataModel.getHeaderToken(),
                      ...
                      REQ_ORDER_TYPE_ID: VAL_ORDERTYPE_LANDLINE] as [String : Any]
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        if textField == txtOperator {
            self.hideKeyboard()
            let operatorView:OperatorView = OperatorView.init(frame: UIScreen.main.bounds, categoryID: HOME_CATEGORY_TYPE.LANDLINE.rawValue)
