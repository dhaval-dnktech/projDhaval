...
class ElectricityBillViewController: UIViewController ,UITextFieldDelegate , UIScrollViewDelegate , UITableViewDelegate,UITableViewDataSource , OperatorViewDelegate , CubberTextFieldDelegate , RegionViewDelegate , AppNavigationControllerDelegate  {

    //MARK: CONSTANT
    internal let TAG_PLUS: Int = 100
    internal let TAG_ELECTRICITY_REGION: Int = 1
   ...
   // var dictOrder = [typeAliasDictionary]
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.SetScreenName(name: F_MODULE_ELE, stclass: F_MODULE_ELE)
        self.sendScreenView(name: F_MODULE_ELE)
    ...
    func showRechargeView(){
        ...
        if isPaidFees {
            ...
            rechargeVC._RECHARGE_TYPE = RECHARGE_TYPE.ELECTRICITY_BILL
            ...
            gtmModel.list = "Electricity Bill Section"
    ...
    func setValueInOperatorField(dictOperator:typeAliasDictionary , dictRegion:typeAliasDictionary , isOrder:Bool) {
        ...
        if !dictRegion.isEmpty {
            for viewBG in self.viewServiceTextFiled.subviews {
                if viewBG.isKind(of: CubberTextField.self) {
                    let _CubberTextField = viewBG as! CubberTextField
                    if _CubberTextField.txtContent.tag - TAG_PLUS == TAG_ELECTRICITY_REGION {
    ...
    fileprivate func callGetLatestOrdersService() {
        ...
        let params = [REQ_HEADER:DataModel.getHeaderToken(),
                      ...
                      REQ_ORDER_TYPE_ID: VAL_ORDERTYPE_ELECTRICITY_BILL] as [String : Any]
    ...
    func cubberTextFieldShouldBeginEditing(_ textField: UITextField) {
          ...
          self.resetBillInfo()
        if textField.tag - TAG_PLUS == TAG_ELECTRICITY_REGION && textField.accessibilityLabel != nil && self.dictElectricityBill_SelectedOperator[RES_region] != nil {
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        if textField == txtOperator {
            self.hideKeyboard()
            let operatorView:OperatorView = OperatorView.init(frame: UIScreen.main.bounds, categoryID: HOME_CATEGORY_TYPE.ELECTRICITY.rawValue)
