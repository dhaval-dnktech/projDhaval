...
class DataCardViewController: UIViewController, UITextFieldDelegate , UIScrollViewDelegate , UITableViewDelegate,UITableViewDataSource , OperatorViewDelegate , CNContactPickerDelegate  ,AppNavigationControllerDelegate {
    ...
    @IBOutlet var constraintLblRemarkHeight: NSLayoutConstraint!
    ...
    fileprivate var dictDataCard_SelectedOperator = typeAliasDictionary()
    fileprivate var dictDataCard_SelectedRegion = typeAliasDictionary()
    fileprivate var dictDataCard_SelectedPlan = typeAliasDictionary()
    ...
    override func viewDidLoad() {
        ...
        if lblRemark.text != "" {
            constraintLblRemarkHeight.constant =  ((lblRemark.text?.textHeight(lblRemark.frame.width, textFont: lblRemark.font)))! + 10 }
        else { constraintLblRemarkHeight.constant =  0 }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.SetScreenName(name: F_MODULE_DATACARD, stclass: F_MODULE_DATACARD)
        self.sendScreenView(name: F_MODULE_DATACARD)
    ...
    @IBAction func btnPrePostpaidAction(_ sender: UIButton) {
        ...
        if !isOrderData {
            isOrderData = false
            dictDataCard_SelectedPlan = typeAliasDictionary()
            txtAmont.text = ""
    ...
    @IBAction func btnOperatorCircleAction() {
        self.hideKeyboard()
        let operatorView:OperatorView = OperatorView.init(frame: UIScreen.main.bounds, categoryID:HOME_CATEGORY_TYPE.DATACARD.rawValue)
    ...
    @IBAction func btnBrowsePlanAction() {
        ...
        if self.dictDataCard_SelectedOperator.isEmpty {
            isBrowsePlan = true
            btnOperatorCircleAction() }
        else {
            self.hideKeyboard()
            let planVC = PlanViewController(nibName: "PlanViewController", bundle: nil)
            planVC.dictOperator = self.dictDataCard_SelectedOperator
            planVC.dictRegion = self.dictDataCard_SelectedRegion
    ...
    func fillOrderFromSlider() {
        ...
        self.dictDataCard_SelectedOperator = dictFillOrder
        self.dictDataCard_SelectedRegion = dictFillOrder
    ...
    internal func onPlanSelection(_ aNotification: Notification) {
        ...
        self.dictDataCard_SelectedPlan = aNotification.object as! typeAliasDictionary
        if self.dictDataCard_SelectedPlan[RES_PlanValue] != nil { txtAmont.text = self.dictDataCard_SelectedPlan[RES_PlanValue] as? String }
        if !viewTopupSpecial.isHidden {
            let isSpelicalType: String = self.dictDataCard_SelectedPlan[RES_isSpelicalType] as! String
    ...
    fileprivate func setMobileRecharge_TopUpAndSpecialRechargeView() {
        if !dictDataCard_SelectedOperator.isEmpty {
            
            let isShowPlanTypeSelect: String = dictDataCard_SelectedOperator[RES_isShowPlanTypeSelect] as! String
    ...
    fileprivate func callSearchMobileOperatorService(_ mobileNo: String) {
        ...
        let params = [REQ_HEADER:DataModel.getHeaderToken(),
                      ...
                      REQ_CATEGORY_ID: HOME_CATEGORY_TYPE.DATACARD.rawValue ]
    ...
    fileprivate func callGetLatestOrdersService() {
        ...
        let params = [REQ_HEADER:DataModel.getHeaderToken(),
                      ...
                      REQ_ORDER_TYPE_ID: VAL_ORDERTYPE_DATACARD] as [String : Any]
    ...
    func showRechargeView() {
        ...
        if isPaidFees {
            ...
            rechargeVC.mobile_CardOperator = self.dictDataCard_SelectedOperator[RES_operatorName] as! String
            rechargeVC.mobile_CardRegionName = self.dictDataCard_SelectedRegion.isEmpty || self.dictDataCard_SelectedRegion[RES_regionName] == nil ? "" : self.dictDataCard_SelectedRegion[RES_regionName] as! String
            ...
            rechargeVC.cart_PlanTypeID = !self.dictDataCard_SelectedPlan.isEmpty ? self.dictDataCard_SelectedPlan[RES_planTypeId] as! String : "0"
            rechargeVC.cart_MobileNo = self.txtMobileNo.text!.trim() //consumer id
            rechargeVC.cart_TotalAmount = self.txtAmont.text!.trim()
            rechargeVC.cart_OperatorId = self.dictDataCard_SelectedOperator[RES_operatorID] as! String
            rechargeVC.cart_RegionId = self.dictDataCard_SelectedRegion.isEmpty || self.dictDataCard_SelectedRegion[RES_regionID] == nil ? "0" : self.dictDataCard_SelectedRegion[RES_regionID] as! String
            rechargeVC.cart_CategoryId = dictOpertaorCategory[RES_operatorCategoryId] as! String
            rechargeVC.cart_RegionPlanId =  dictDataCard_SelectedPlan.isEmpty ? "0" : dictDataCard_SelectedPlan[RES_regionPlanID] as! String
            ...
            rechargeVC.stImageUrl =  self.dictDataCard_SelectedOperator[RES_image] != nil ? self.dictDataCard_SelectedOperator[RES_image] as! String : ""
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.dictDataCard_SelectedOperator = dictOrder
        self.dictDataCard_SelectedRegion = dictOrder
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if resultingString.isEmpty
        {
            if textField == txtMobileNo {
                self.dictDataCard_SelectedOperator = typeAliasDictionary()
                self.dictDataCard_SelectedRegion = typeAliasDictionary()
        ...
        else if textField == txtAmont
        {
            ...
            self.dictDataCard_SelectedPlan = typeAliasDictionary()
            return RET
        }
    ...
    func onOperatorView_Selection(_ dictOperator: typeAliasDictionary, dictRegion: typeAliasDictionary) {
        self.dictDataCard_SelectedOperator = dictOperator
        self.dictDataCard_SelectedRegion = dictRegion
